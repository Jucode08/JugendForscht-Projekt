...
def merge_and_queue(text, language):
    global puffer_text, last_commit

    puffer_text += " " + text
    puffer_text = puffer_text.strip()

    # Commit-Regeln
    if puffer_text.endswith((".", "!", "?")) or len(puffer_text.split()) > 7 or t.time() - last_commit > commit_timeout:
        translatedText = translate(puffer_text, language)
        if not translatedText:
            return
        print(f"übersetzter Text: {translatedText}")
        
        filename = uuid.uuid4().hex + ".mp3"
        tts_queue.put((translatedText, filename))
        puffer_text = ""
        last_commit = t.time()
