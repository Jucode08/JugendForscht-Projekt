...
import argparse
parser = argparse.ArgumentParser(description='Start Vosk Speech-to-Text test with configuration options.')
...
logging.basicConfig(
    filename="transcribe_VAD.log" if args.log_file else None,
    level=logging.DEBUG,
...
with sd.RawInputStream(samplerate=SAMPLE_RATE, blocksize=BLOCK_SIZE, dtype="int16",
    ...
    if LOGGING: logger.debug(f">>> Current State: {STATE}")
            ...
            if STATE == "WAITING":
                ...
                for i in range(0, len(data), frame_len):
                    ...
                    if len(frame) == frame_len and vad.is_speech(frame, SAMPLE_RATE):
                        if LOGGING: logger.debug(">>> Speech detected. Switching to TRANSCRIBING...")
                        STATE = "TRANSCRIBING"
                ...
                if last_speech_time and (time() - last_speech_time) > POST_SPEECH_SILENCE:
                    if LOGGING : logger.debug(">>> Silence detected. Switching to WAITING...")
                    STATE = "WAITING"
                    prebuffer.clear()
                    last_speech_time = None
